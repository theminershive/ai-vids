...
ENFORCED_WORKFLOW_PATH = os.getenv("ENFORCED_WORKFLOW_PATH", "./mod2/workflow.json").strip()
...
def generate_image_once(prompt: str, model: dict) -> str:
    ...
    for _ in range(LEONARDO_POLL_ATTEMPTS):
        ...
        poll = requests.get(
            f"{LEONARDO_API_ENDPOINT}/generations/{gen_id}",
            headers=HEADERS,
...
def comfyui_build_workflow(prompt: str, section_idx: int) -> dict:
    ...
    # Optional size/step overrides from .env (enforce min height unless COMFYUI_ALLOW_SMALL=1)
