...
def generate_and_download_images(script: dict, visuals_dir: Path) -> dict:
    ...
    theme = (script.get("settings", {}) or {}).get("image_theme") or script.get("image_theme")
    book_title = script.get("reference") or script.get("title")
    ...
    for section_idx, section in enumerate(sections, start=1):
        ...
        prompt = rewrite_prompt(prompt, theme=theme, book_title=book_title)
...
def main():
    parser = argparse.ArgumentParser(description="Bookmaker pipeline")
    parser.add_argument("--force-assemble", action="store_true", help="Run assemble.py to generate a new topic JSON")
    args = parser.parse_args()

    config = load_config()
    ensure_dirs(config)

    ready_files = sorted(config.paths.ready_dir.glob("*.json"), key=_episode_sort_key)
    if args.force_assemble or not ready_files:
