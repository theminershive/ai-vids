...
def _fallback_video_size() -> tuple[str, str]:
    size = (config.video.size or "").lower()
    if "x" not in size:
        return "", ""
    w, h = size.split("x", 1)
    return w.strip(), h.strip()

if not COMFYUI_WIDTH.isdigit() or not COMFYUI_HEIGHT.isdigit():
    fb_w, fb_h = _fallback_video_size()
    if not COMFYUI_WIDTH.isdigit() and fb_w.isdigit():
        COMFYUI_WIDTH = fb_w
    if not COMFYUI_HEIGHT.isdigit() and fb_h.isdigit():
        COMFYUI_HEIGHT = fb_h
...
def rewrite_prompt(original_prompt: str, theme: str | None = None, book_title: str | None = None) -> str:
    if not original_prompt:
        return original_prompt

    if LOCAL_PROMPT_REWRITE_URL:
        try:
            theme_line = f"\nBook theme: {theme}" if theme else ""
            ...
            payload = {
                ...
                "prompt": (
                    ...
                    "Incorporate the provided book theme into the visual style and atmosphere. "
                    "Return only the rewritten prompt text.\n\n"
                    f"{original_prompt}{title_line}{theme_line}"
                ),
                "stream": False,
            }
            ...
            resp = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    },
                    {
                        "role": "user",
                        "content": (
                            f"Rewrite this image prompt to be safe:\n{original_prompt}\n"
                            f"Book title: {book_title or ''}\n"
                            f"Book theme: {theme or ''}"
                        ),
                    },
                ],
                max_tokens=200,
